...
void UP1SpringArmComponent::UpdateDesiredArmLocation(bool bDoTrace, bool bDoLocationLag, bool bDoRotationLag, float DeltaTime)
{
	FRotator DesiredRot = GetDesiredRotation();

	// bUsePawnControlRotation을 사용하지 않고, yaw 값만 Controller에 영향받도록 커스텀.
	if (APawn* OwningPawn = Cast<APawn>(GetOwner()))
	{
		const FRotator PawnViewRotation = OwningPawn->GetViewRotation();
		if (DesiredRot.Yaw != PawnViewRotation.Yaw)
		{
			DesiredRot.Yaw = PawnViewRotation.Yaw;
		}
	}
